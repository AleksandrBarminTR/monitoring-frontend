...
export default class MergeRequestParticipant extends Component {
  ...
  loadRequests() {
    this.setState(state => {
      ...
      loadMergeRequestsList(this.props.participant.id, this.state.props).then(requests => {
        this.setState(state => {
          state.loading = false
        ...
        <Card>
          ...
          <Accordion.Collapse eventKey="content">
            <Card.Body>
              ...
              <MergeRequestProps properties={this.state.props} onChange={this.loadNewProperties} />
              {content}
